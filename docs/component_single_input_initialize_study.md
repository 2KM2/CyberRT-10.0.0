...
## 22. `component` / `module` / `dag` / `process` 层级图

如果你已经理解了单输入 component 的初始化链路，下一步最容易混淆的就是这四个概念：
...
rectangle "Process\n操作系统进程\n真正跑起来的执行实体" as Process {
  rectangle "Module\n启动单元\n指定 process_name 和 dag_conf" as Module {
    rectangle "DAG\n接线图 / 装配图\n描述加载哪些 component、读哪些 channel" as DAG {
      rectangle "Component A\n如 LidarDriverComponent\n真正干活的业务类" as ComponentA
      rectangle "Component B\n如 VelodyneConvertComponent\n真正干活的业务类" as ComponentB
      rectangle "Component C\n如 CompensatorComponent\n真正干活的业务类" as ComponentC
    }
  }
}

Process -down-> Module : 包含 / 运行
Module -down-> DAG : 引用 dag_conf
DAG -down-> ComponentA : 装配
DAG -down-> ComponentB : 装配
DAG -down-> ComponentC : 装配

note right of ComponentA
C++ 类
实现 Init() / Proc()
是最小业务执行单元
end note
...
`component` 是最小的业务执行单元，就是你在 C++ 里真正写的类，比如：
...
> 这些 component 怎么被组织起来、怎么连起来？

#### 3. `module`

`module` 通常是在 launch 里定义的启动单元。它会指定：
...
- `dag`：[velodyne_lidar.dag](/home/zkm/CyberRT-10.0.0/modules/drivers/lidar/dag/velodyne_lidar.dag:1)
- `module`：[driver.launch](/home/zkm/CyberRT-10.0.0/modules/drivers/lidar/launch/driver.launch:1) 里定义的 `lidar_driver`
- `process`：`process_name` 对应的实际 OS 进程

所以要特别注意：

> `component` 分开，不等于 `process` 分开。

raw data 采集和 pointcloud 转换确实是两个不同 `component`，但在当前 DAG/launch 配置下，它们很可能仍然运行在同一个 `process` 里。
